...
HI_S32  main(HI_S32 argc,HI_CHAR *argv[])
{
    ...
    if (9 == argc)
	{
        ...
        g_GpioType = strtol(argv[8],NULL,0);
    }
	else if (8 == argc)
    {
        g_TimerCount  = strtol(argv[1],NULL,0);
    ...
    {
        printf("Usage: %s [TimerCount] [Display] [Running Count] [Scene] [IR Type] [Keyled Type]\n"
	...
	if (g_GpioType == 1)
	{
		...
		devType.gptype = HI_UNF_GPIO_INTTYPE_HIGH; 
	}
    else if (g_GpioType == 0)
    {
    ...
    if (devType.irtype == HI_UNF_IR_CODE_RAW)
    {
        ...
        wakeup.u32IrPowerKey0[0] = 0x639cff00; // nec sample: nec protocol
        wakeup.u32IrPowerKey1[0] = 0x0;
        wakeup.u32IrPowerKey0[1] = 0xf20dfb04; // rc-8a     : nec protocol
        wakeup.u32IrPowerKey1[1] = 0x0;
        wakeup.u32IrPowerKey0[2] = 0x58595a53; // nec full dezhou protocol
        wakeup.u32IrPowerKey1[2] = 0x5750;
        wakeup.u32IrPowerKey0[3] = 0x0000000c; // rc5 protocol
        wakeup.u32IrPowerKey1[3] = 0x0;
        wakeup.u32IrPowerKey0[4] = 0x8010260c; // tongzhou  :  rc6 protocol
        wakeup.u32IrPowerKey1[4] = 0x00000000;
        wakeup.u32IrPowerKey0[5] = 0x544c5a53; // nec full 2 headers
        wakeup.u32IrPowerKey1[5] = 0xb3;
    }
    ...
    {
        wakeup.u32IrPmocNum = 1;
        wakeup.u32IrPowerKey0[0] = 0xf40b0e0e;
        wakeup.u32IrPowerKey1[0] = 0x0;
    }
    else
    {
